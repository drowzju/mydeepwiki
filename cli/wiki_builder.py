...
Examples:
  ...
  python wiki_builder.py /path/to/local/repo --type local --language zh
    ...
    parser.add_argument("--provider", default="aliyun_coding",
                       help="Model provider (default: aliyun_coding)")
    parser.add_argument("--model", default="kimi-k2.5",
                       help="Model name (default: kimi-k2.5)")
    ...
    parser.add_argument("--rag", action="store_true",
                       help="Enable RAG (requires embedder API keys). Disabled by default")
    parser.add_argument("--no-cache", action="store_true",
                       help="Don't save to server cache")

    args = parser.parse_args()

    # Note: RAG is disabled by default for simplicity
    # Use --rag to enable it (requires embedder configuration)
